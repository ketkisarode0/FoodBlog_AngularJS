...
const booksModule = angular
  ...
  .controller("tableCtrl", [
    ...
    "$http",
    "bookService",
    function($scope, $http, bookService) {
      bookService.validate().then(function success(response) {
      ...
      $scope.addBook = function() {
        ...
        bookService.addBook($scope.newBook).then(function success(response) {
          console.log(response.data.message);
      ...
      $scope.deleteBook = function(id, index) {
        console.log(id);
        bookService.deleteBook(id).then(function success(response) {});
